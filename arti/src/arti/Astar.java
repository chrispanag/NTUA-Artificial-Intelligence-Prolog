...
public class Astar {
	...
	class searchNode implements Comparable<searchNode> {
		...
		public searchNode(double g, Node node, ArrayList<Node> pastPath) {
			this.maxSpeed = node.heuristic(jip);
			double distance = node.coord.distanceFrom(target.coord)/this.maxSpeed;
	...
	public Path start() {
		...
		while (!open.isEmpty()) {
			...
			children_ids.forEach((id) -> {
				...
				edges.add(new Edge(n, n.line_id, n.coord.distanceFrom(selected1.node.coord), selected1.maxSpeed));
			});
			...
			edges.forEach((e) -> {
				...
				searchNode node = new searchNode(selected1.g + e.length/e.maxSpeed, e.to, path);
