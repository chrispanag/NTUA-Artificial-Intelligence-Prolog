...
public class Test {
	...
	public static void main(String[] args) {
		// Read Client
		CSVReader csvClient = new CSVReader("client.csv");
		ArrayList<ArrayList<String>> clientData = csvClient.readCSV();
		Client client = null;
		Iterator<ArrayList<String>> clientData_it = clientData.iterator();
		while (clientData_it.hasNext()) {
			ArrayList<String> p = clientData_it.next();
			client = new Client(p);
		}
		
		// Read Taxis
		CSVReader csvTaxis = new CSVReader("taxis.csv");
		ArrayList<ArrayList<String>> taxiData = csvTaxis.readCSV();
		ArrayList<Taxi> taxis = new ArrayList<Taxi>();
		Iterator<ArrayList<String>> taxiData_it = taxiData.iterator();
		while (taxiData_it.hasNext()) {
			ArrayList<String> p = taxiData_it.next();
			taxis.add(new Taxi(p));
		}
		...
		if (compile.equals("yes")) {
			...
			PrologCreator plCreator_taxis = new PrologCreator("./prolog/taxis.pl");
			writeTaxiPredicates(taxis, plCreator_taxis);
			
			plCreator_taxis.close();
		...
		Node start = Node.getNearestNodeRoad(jip, client.start, nodes);
		Node end = Node.getNearestNodeRoad(jip, client.dest, nodes);
	...
	public static void writeTaxiPredicates (ArrayList<Taxi> taxis, PrologCreator plCreator) {
		Iterator<Taxi> taxi_it = taxis.iterator();
		
		while (taxi_it.hasNext()) {
			Taxi t = taxi_it.next();
			if (t.available) {
				Predicate k = new Predicate("available", new String[] {"id"+ t.id});
				plCreator.writePredicate(k);
			}
		}
	}
